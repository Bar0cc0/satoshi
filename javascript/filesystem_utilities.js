...
/** 
 * Creates a gdrive folder for project
 * 
 * @param {string} project ID
 * @return {object} project folder ID 
 */
function createNewProjectFolder(projectID) {
    let folder = DriveApp.getFolderById(getFolder_(PROJECTS_FOLDER).getId());
    const newFolderId = Drive.Files.insert({
      parents: [{id: `${folder.getId()}`}],
      title: projectID,
      mimeType: 'application/vnd.google-apps.folder'
    }, null, {supportsAllDrives: true}).id;
    console.log(`Folder ${projectID} has been created`);
    return newFolderId;
    
  }
  
  /**
   * Fetch file by name in a specific folder.
   * Returns true if file not found, false if an error occured.
   * 
   * @param {string} file name
   * @param {object} folder object
   * @return {object} file object
   */
  function getFile_(file, folder) {
    try {
      var f = folder.getFilesByName(file); 
      if (f.hasNext()) {
        return f.next()
      };
    return true;
  ...
  function getSpreadSheet_(fileName, objFolder) {
    ...
    const folderAppPrimary = getApplicationFolder_(DB_FOLDER);
